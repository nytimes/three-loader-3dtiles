...
`);const g=new URLSearchParams(document.location.search),l=document.querySelector("#canvas-parent"),C=document.querySelector("#stats-widget"),F=new X,p=new M(35,1,.01,1e3),u=new Z;u.outputColorSSpace=B;const ot=new Q,at=new G(p,F);let m;g.get("orbit")?(m=new W(p,l),m.listenToKeyEvents(document.body),p.position.set(0,200,0),m.update()):(m=new st(at,{panDegreeFactor:2,wheelScaleFactor:.01,keyboardScaleFactor:.015,keyboardDampFactor:0}),m.enable());l.appendChild(u.domElement);const y=new I;y.domElement.style.position="absolute";y.domElement.style.top="10px";y.domElement.style.left="10px";l.appendChild(y.domElement);let E,T;g.get("tilesetUrl")&&(document.querySelector("#example-desc").style.display="none");rt();async function rt(){const s=await _.load({url:g.get("tilesetUrl")??"https://int.nyt.com/data/3dscenes/ONA360/TILESET/0731_FREEMAN_ALLEY_10M_A_36x8K__10K-PN_50P_DB/tileset_tileset.json",renderer:u,options:{dracoDecoderPath:"https://cdn.jsdelivr.net/npm/three@0.160.0/examples/jsm/libs/draco",basisTranscoderPath:"https://cdn.jsdelivr.net/npm/three@0.160.0/examples/jsm/libs/basis",maximumScreenSpaceError:g.get("sse")??48}}),{model:t,runtime:e}=s;t.rotation.set(-Math.PI/2,0,Math.PI/2),g.get("tilesetUrl")||t.position.set(-1,4,-16),E=e,F.add(t),T=new K(e.getStats(),{container:C}),C.style.visibility="visible"}function z(s){const t=ot.getDelta();m.update(s),E&&E.update(t,u,p),T&&T.update(),u.render(F,p),y.update(),window.requestAnimationFrame(z)}O();function O(){u.setSize(l.clientWidth,l.clientHeight),p.aspect=l.clientWidth/l.clientHeight,p.updateProjectionMatrix()}window.addEventListener("resize",O);z();
