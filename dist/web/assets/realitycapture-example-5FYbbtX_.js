...
`);const u=new URLSearchParams(document.location.search),l=document.querySelector("#canvas-parent"),C=document.querySelector("#stats-widget"),F=new Z,p=new M(35,1,.01,1e3),z=new B,g=new _;g.outputColorSSpace=Q;const rt=new G,ht=new $(p,F);let m;u.get("orbit")?(m=new I(p,l),m.listenToKeyEvents(document.body),p.position.set(0,200,0),m.update()):(m=new at(ht,{panDegreeFactor:2,wheelScaleFactor:.01,keyboardScaleFactor:.015,keyboardDampFactor:0}),m.enable());l.appendChild(g.domElement);const y=new K;y.domElement.style.position="absolute";y.domElement.style.top="10px";y.domElement.style.left="10px";l.appendChild(y.domElement);let E,T;u.get("tilesetUrl")&&(document.querySelector("#example-desc").style.display="none");dt();async function dt(){const s=await H.load({url:u.get("tilesetUrl")??"https://int.nyt.com/data/3dscenes/ONA360/TILESET/0731_FREEMAN_ALLEY_10M_A_36x8K__10K-PN_50P_DB/tileset_tileset.json",renderer:g,options:{dracoDecoderPath:"https://cdn.jsdelivr.net/npm/three@0.160.0/examples/jsm/libs/draco",basisTranscoderPath:"https://cdn.jsdelivr.net/npm/three@0.160.0/examples/jsm/libs/basis",maximumScreenSpaceError:u.get("sse")??48,resetTransform:!0}}),{model:t,runtime:e}=s;t.rotation.set(-Math.PI/2,0,Math.PI/2),u.get("tilesetUrl")||t.position.set(-1,4,-16),E=e,F.add(t),T=new N(e.getStats(),{container:C}),C.style.visibility="visible"}function O(s){const t=rt.getDelta();m.update(s),E&&E.update(t,z,p),T&&T.update(),g.render(F,p),y.update(),window.requestAnimationFrame(O)}V();function V(){g.setSize(l.clientWidth,l.clientHeight),z.set(l.clientWidth,l.clientHeight),p.aspect=l.clientWidth/l.clientHeight,p.updateProjectionMatrix()}window.addEventListener("resize",V);O();
